...
def chat_interface():
    ...
    if 'role' in st.session_state:
                ...
                try:
                    with st.spinner("Generating the answer..."):
                        ...
                        if results:
                            ...
                            chat_context = "\n".join([f"User: {msg['question']}\nNOVA: {msg['answer']}" for msg in
                                                      st.session_state.chat_history])
                            chat_input = f"{chat_context}\nUser: {query}"

                            completion = client.chat.completions.create(
                                model="llama3-8b-8192",
                                messages=[{"role": "system", "content": "You are NOVA, an organizational assistant."},
                                          {"role": "user",
                            ...
                            st.markdown(f"<div style='background-color: #262730; padding: 10px; border-radius: 5px;'><strong>NOVA:</strong> {answer}</div>", unsafe_allow_html=True)
        ...
        for msg in st.session_state.chat_history:
            ...
            # Only allow admin to upload PDFs in the sidebar
